...
namespace WindowsFormsApp1
{
    public partial class Form_student : Form
    {
        ...
        public Form_student(UserInfo userInfo)
        {
            InitializeComponent();
            this.loginUser = userInfo;
            lbl姓名.Text = loginUser.姓名 + "";
            lbl班級.Text = loginUser.班級 + "";
        ...
        private void lbl班級_Click(object sender, EventArgs e)
        {

        }

        private void lbl姓名_Click(object sender, EventArgs e)
        {
        ...
        private void btn加入訂單_Click(object sender, EventArgs e)
        {
           ...
           string orderTime = DateTime.Now.ToShortDateString();
            if (lbox菜單.SelectedItem != null)
            {
                ...
                cmd.Parameters.AddWithValue("@ClassName", lbl班級.Text);
                cmd.Parameters.AddWithValue("@StudentName", lbl姓名.Text);
        ...
        public void UpData()
        {
            if (lbl姓名.Text != null)
            {
                ...
                adp.SelectCommand.Parameters.AddWithValue("@StudentName", lbl姓名.Text);
                DataTable dt = new DataTable();
                adp.Fill(dt);
                con.Close();
                this.dvgshow.DataSource = dt;

                if (dvgshow.Rows.Count > 1)
                {   //更新價格
        ...
        private void btn清空品項_Click(object sender, EventArgs e)
        {
            if (dvgshow.Rows.Count > 0)
            {
                ...
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter(strSQL, con);
                da.SelectCommand.Parameters.AddWithValue("@Name", lbl姓名.Text);
                int row = da.SelectCommand.ExecuteNonQuery();
                con.Close();
        ...
        private void btn搜尋_Click(object sender, EventArgs e)
        {
            if (txt輸入搜尋.Text != "" && Cob搜尋選項.SelectedItem != null)
            {
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                string strSQL = $"select * from orders where 姓名 = '{lbl姓名.Text}' and {Cob搜尋選項.SelectedItem.ToString()} like '%{ txt輸入搜尋.Text}%';";
               // select* from orders where 姓名 = '蔡禕霖' and 店家 like '%正忠%';
                ...
                {   //更新價格
                    con = new SqlConnection(myDBConnectionString);
                    con.Open();
                    strSQL = $"select sum (總價) as total from orders where  姓名 = '{lbl姓名.Text}' and { Cob搜尋選項.SelectedItem.ToString() } like '%{ txt輸入搜尋.Text}%'; ";
                    SqlCommand cmd = new SqlCommand(strSQL, con);
        ...
        private void btn送出訂單_Click(object sender, EventArgs e)
        {
            if (dvgshow.Rows.Count > 0)
            {
                ...
                listorderfile.Add("************" + " " + lbl班級.Text + "  " + lbl姓名.Text + " " + "便當訂購單*************\n");
                listorderfile.Add("");
                listorderfile.Add("                        訂購時間:" + DateTime.Now);
                listorderfile.Add("訂購品項:");
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                string strSQL = "select 店家, 菜名, 數量, 單價, 總價, 日期 from orders where 班級=@ClassName and 姓名=@StudentName;";
                ...
                while (reader.Read())
                {
                    ...
                    if (lbl姓名.Text != "")
                    {
                        ...
                        listorderfile.Add((string)reader["店家"] + " " + (string)reader["菜名"] + " " + ((int)reader["數量"]).ToString() + "個   單價:" + ((int)reader["單價"]) + "  共" + ((int)reader["總價"]).ToString() + "元");
                ...
                strSQL = "select sum(總價) as totalprice from orders where 姓名 =@StudentName;";
                ...
                if (reader.Read())
                {
                    ...
                    listorderfile.Add("");
                    listorderfile.Add("-------------------------------------------");
                    listorderfile.Add("總計金額共" + ((int)reader["totalprice"]).ToString() + "元");
