...
{
    public partial class Form_classSever : Form
    {
        ...
        public Form_classSever()
        {
            InitializeComponent();
        }

        public Form_classSever(UserInfo userInfo)
        {
            InitializeComponent();
        ...
        private void Form_classSever_Load(object sender, EventArgs e)
        {
            ...
            cob姓名.Items.Clear();
            string strFieldClass = lbl班級.Text;
            ...
            reader2.Close();
            con2.Close();
            //  GlobalVar.班級 = cob班級.SelectedItem.ToString();
        ...
        private void cob姓名_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void btn送出訂單_Click(object sender, EventArgs e)
        {
            ...
            if (dvgshow.Rows.Count > 0)
            {
                ...
                listorderfile.Add("**************" + "    " + lbl班級.Text + "   "  + "便當訂購單***************\n");
                listorderfile.Add("");
                listorderfile.Add("                        訂購時間:" + DateTime.Now);
                listorderfile.Add("訂購品項:");
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                string strSQL = $"select 姓名, 店家, 菜名, 數量, 單價, 總價, 日期 from orders where 班級 = '{lbl班級.Text}' and { Cob搜尋選項.SelectedItem.ToString() } like '%{ txt輸入搜尋.Text}%' ";
               ...
               // cmd.Parameters.AddWithValue("@StudentName", cob姓名.SelectedItem);
                ...
                while (reader.Read())
                {
                    ...
                    if (cob姓名.SelectedItem != null)
                    {
                        ...
                        listorderfile.Add((string)reader["姓名"] + " "+(string)reader["店家"] + " " + (string)reader["菜名"] + " " + ((int)reader["數量"]).ToString() + "個   單價:" + ((int)reader["單價"]) + "  共" + ((int)reader["總價"]).ToString() + "元");
                ...
                strSQL = $"select sum (總價) as total from orders where  班級 = '{lbl班級.Text}' and { Cob搜尋選項.SelectedItem.ToString() } like '%{ txt輸入搜尋.Text}%'; ";
        ...
        private void Form_classSever_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void btn搜尋_Click(object sender, EventArgs e)
        {
            if (txt輸入搜尋.Text != "" && Cob搜尋選項.SelectedItem != null)
            {
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                string strSQL = $"select * from orders where 班級 = '{lbl班級.Text}' and {Cob搜尋選項.SelectedItem.ToString()} like '%{ txt輸入搜尋.Text}%';";
